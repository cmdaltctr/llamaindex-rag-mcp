"""Tests for the metadata extraction module (omrg.metadata_extractor)."""
...
class TestKeywordExtraction:
    ...
    def test_single_category_match(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata import extract_metadata_async
    ...
    def test_biology_match(self, monkeypatch) -> None:
        ...
        result = asyncio.run(
            extract_metadata_async("The crispr gene editing tool revolutionises cancer research.")
        )
    ...
    def test_programming_match(self, monkeypatch) -> None:
        ...
        result = asyncio.run(
            extract_metadata_async("The Rust compiler uses an LLVM backend for code generation.")
        )
...
class TestCustomKeywordRules:
    ...
    def test_invalid_json_falls_back_to_defaults(
        ...
        result = asyncio.run(
            extract_metadata_async("The transformer attention mechanism is key to modern AI.")
        )
...
class TestLlamaindexStub:
    ...
    def test_llamaindex_falls_back_to_ollama_when_not_installed(
        ...
        async def _fake_ollama(text: str, file_name: str = "", settings=None) -> dict:
            return {
                "category": "biology",
                "keywords": ["protein", "deep_learning"],
                "summary": "A biology paper.",
            }

        monkeypatch.setattr(
            "omrg.core.metadata.ollama._extract_ollama_async",
            _fake_ollama,
        )
        ...
        assert any(
            r.levelno == logging.WARNING and "not installed" in r.message.lower()
            for r in caplog.records
        )
...
class TestUnknownMode:
    ...
    def test_unknown_mode_falls_back_to_keyword(
        ...
        # Must log a WARNING
        assert any(
            r.levelno == logging.WARNING and "unknown" in r.message.lower() for r in caplog.records
        )
...
class TestOllamaExtraction:
    ...
    def test_ollama_unreachable(self, monkeypatch, caplog) -> None:
        ...
        # Must log a WARNING
        assert any(
            r.levelno == logging.WARNING and "ollama classification failed" in r.message.lower()
            for r in caplog.records
        )
...
class TestHybridCategoryTaxonomy:
    ...
    def test_chromadb_query_failure_falls_back_to_seeds(self, monkeypatch, caplog) -> None:
        ...
        assert any(
            r.levelno == logging.WARNING and "failed to query vector store" in r.message.lower()
            for r in caplog.records
        )
    ...
    def test_category_normalisation_deduplicates(self, monkeypatch) -> None:
        ...
        lines = prompt.split("\n")
        category_lines = [line for line in lines if line.strip().startswith("- ")]
        cats_in_prompt = [line.strip("- ").strip() for line in category_lines]
...
class TestCategoryNormalisation:
    ...
    def test_lowercase_and_underscores(self) -> None:
        from omrg.core.metadata._common import _normalise_category
...
class TestSeedCategories:
    ...
    def test_includes_all_keyword_categories(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata.taxonomy import _get_seed_categories
...
class TestLlamaindexExtraction:
    ...
    def test_successful_extraction(self, monkeypatch, caplog) -> None:
        ...
        from omrg.core.metadata.llamaindex import _aggregate_llamaindex_metadata

        async def fake_extract_llamaindex_async(
            text: str, file_name: str = "", settings=None
        ) -> dict:
    ...
    def test_extraction_failure_falls_back_to_ollama(self, monkeypatch, caplog) -> None:
        ...
        monkeypatch.setattr(
            sys.modules["llama_index.llms.ollama"],
            "Ollama",
            MagicMock(),
        )
        ...
        # Must log a WARNING mentioning local mode
        assert any(
            r.levelno == logging.WARNING and "falling back to local mode" in r.message.lower()
            for r in caplog.records
        )
    ...
    def test_double_degradation_llamaindex_and_ollama_both_fail(
        ...
        assert any(
            r.levelno == logging.WARNING and "local mode" in r.message.lower()
            for r in caplog.records
        )
...
class TestCoverageGaps:
    ...
    def test_strip_llm_prefix_empty_string(self) -> None:
        ...
        from omrg.core.metadata._common import _strip_llm_prefix
    ...
    def test_strip_llm_prefix_bold_markers(self) -> None:
        ...
        assert (
            _strip_llm_prefix('** "Hallucinations in Language Models" **')
    ...
    def test_load_keyword_rules_empty_list(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata.keyword import _load_keyword_rules

        result = _load_keyword_rules(EffectiveSettings(metadata=MetadataBlock(keyword_rules="[]")))
        assert result == []
    ...
    def test_load_keyword_rules_missing_keys(self, monkeypatch) -> None:
        ...
        _rules_settings = EffectiveSettings(
            metadata=MetadataBlock(keyword_rules='[{"pattern": "foo"}]')
        )
        from omrg.core.metadata.keyword import _DEFAULT_KEYWORD_RULES, _load_keyword_rules
    ...
    def test_extract_keyword_empty_rules(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata.keyword import _extract_keyword
    ...
    def test_extract_keyword_invalid_regex_skipped(self, monkeypatch, caplog) -> None:
        ...
        bad_rules = '[{"pattern": "[invalid(", "category": "broken"}, {"pattern": "neural", "category": "AI"}]'  # noqa: E501
        _rules_settings = EffectiveSettings(metadata=MetadataBlock(keyword_rules=bad_rules))
        from omrg.core.metadata.keyword import _extract_keyword
    ...
    def test_gather_existing_categories_collection_error_skipped(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata.taxonomy import _gather_existing_categories
    ...
    def test_build_ollama_prompt_empty_merged_taxonomy(self, monkeypatch) -> None:
        ...
        from omrg.core.metadata.ollama import _build_ollama_prompt
    ...
    def test_parse_ollama_json_non_list_keywords(self) -> None:
        ...
        from omrg.core.metadata.ollama import _parse_ollama_json_response
...
class TestOllamaMarkdownFenceStripping:
    ...
    def test_strip_json_fence(self) -> None:
        from omrg.core.metadata.ollama import _strip_markdown_fence
...
class TestOllamaRetry:
    ...
    def _ollama_mode(self, monkeypatch) -> None:
        ...
        import omrg.core.metadata.ollama as _ollama
...
def _capturing_async_client(monkeypatch, side_effects: list) -> list[dict]:
    ...
    async def _post(*args, **kwargs):
        ...
        if isinstance(effect, int):
            ...
            raise httpx.HTTPStatusError(f"HTTP {effect}", request=request, response=response)
        if isinstance(effect, Exception):
            raise effect
        mock_response = MagicMock()
        mock_response.raise_for_status = MagicMock()
...
_GOOD_RESPONSE = json.dumps({"category": "ai", "keywords": ["transformer"], "summary": "ok"})
...
def _no_sleep(monkeypatch) -> list[float]:
    ...
    import omrg.core.metadata._common as _common
...
class TestStructuredOutputEnforcement:
    ...
    def test_fenced_response_still_parses_under_enforcement(self, monkeypatch) -> None:
...
class TestOpenRouterStructuredOutputDowngrade:
    ...
    def test_downgrades_and_succeeds(self, monkeypatch, status: int) -> None:
        ...
        payloads = _capturing_async_client(monkeypatch, [status, _GOOD_RESPONSE])
    ...
    def test_downgrade_logs_at_info(self, monkeypatch, caplog) -> None:
        ...
        assert any(
            r.levelno == logging.INFO and "structured outputs" in r.message.lower()
            for r in caplog.records
        )

    @pytest.mark.parametrize("status", [429, 401, 403, 500])
    def test_non_parameter_failures_keep_the_schema(self, monkeypatch, status: int) -> None:
    ...
    def test_no_downgrade_when_budget_is_a_single_attempt(self, monkeypatch) -> None:
    ...
    def test_downgrade_happens_at_most_once(self, monkeypatch, caplog) -> None:
        ...
        assert result == {"category": "uncategorised", "keywords": [], "summary": ""}
        assert any(
            r.levelno == logging.WARNING and "openrouter classification failed" in r.message.lower()
            for r in caplog.records
        )
