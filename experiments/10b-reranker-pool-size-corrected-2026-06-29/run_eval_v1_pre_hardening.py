"""SUPERSEDED pre-hardening runner — INVALID for decision evidence.

Defect (voided by the Stage 4 pre-calibration audit, design D17): this
v1 runner executed dense-only reranker-on cells while the protocol
declared a dense x hybrid matrix, so the hybrid hypothesis was claimed
but never tested; it shared no reranker-off controls across modes; and
the fetch_k pools lacked 150, the current post-ADR-021
production-equivalent pool at top_k=50.

Retained for provenance only (Stage 4 task 4.3.1, design D17). The
repaired combined factorial lives in ``run_eval.py``; its
machine-readable cell matrix is ``plan.json``.
