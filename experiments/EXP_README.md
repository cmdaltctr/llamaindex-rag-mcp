...
| 20       | [Citation faithfulness](./20-citation-faithfulness-2026-09-02/)                                    | 2026-09-02 | READY TO RUN          | `add-grounded-answer-synthesis-3` task 7.2 / security F4 follow-up. Does an LLM judge detect unsupported claims (recall ≥ 0.80 overall and per adversarial class, false rejection ≤ 0.10 overall and on paraphrases, P95 ≤ 5 s) where a lexical baseline cannot (gap ≥ 0.20 at the same false-rejection budget)? Decides whether `ok` gains claim-support verification, verification ships as opt-in diagnostics, or the referential-only guarantee stays. Ground truth frozen at 120 triples and audited by a-review (30 claims reworded: 12 contradicted made explicitly exclusive, 5 invented made purely additive, 13 paraphrases agent-preserving; amendments recorded in protocol). Cell 20A recomputed post-audit: constrained H2 reference @0.7 recall 0.617 / FR 0.017 with contradicted recall 0.0 (lexical is blind to it); unconstrained @0.9 recall 0.967 / FR 0.233. Operator review of ground-truth.json gates the judge runs |
