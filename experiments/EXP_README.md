...
| 24       | [OCR routing evaluation](./24-ocr-routing-eval-2026-09-08/)                                        | 2026-09-08 | FAIL (run 1 — fixture defect) | `improve-rag-input-quality-5` task 5.1. Four of five frozen gates PASS (0 failures, 3/3 fast-path byte-identity, worker 104.6 s/page ≤ 180, routing 2.26 ms ≤ 50). Structure-marker gate FAILS: both routed fixtures are 605-byte blank PDFs (no image data, no text operators) — nothing to OCR; worker correctly returns a placeholder. Per task 5.5 the gate stands; default stays off, ADR-062 Proposed. Remediation: repair the task 1.1 fixtures with real rasterised content, re-run against the same frozen gates |
