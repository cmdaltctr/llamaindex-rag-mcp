...
| 20       | [Citation faithfulness](./20-citation-faithfulness-2026-09-02/)                                    | 2026-09-02 | PASS                  | `add-grounded-answer-synthesis-3` task 7.2 / security F4 follow-up. All gates pass: GLM-5.3 judge recall 1.0, false rejection 0.0, P95 3.33 s, unparseable 0, cross-model agreement 1.0 (DeepSeek V4 Flash 0731). Lexical gap 0.383 (lexical 0.617 vs judge 1.0 at the same false-rejection budget). Amendment 5: judge switched from local qwen3:4b (262K context exhausted 43 GB) to cloud (Z.AI GLM-5.3 + OpenRouter DeepSeek). Decision: propose claim-verification stage behind `ANSWER__VERIFY_CLAIMS=true`; F4 mitigated |
| 21       | [LanceDB engine literal-faithfulness inventory](./21-lancedb-literal-faithfulness-2026-09-02/)    | 2026-09-02 | PASS                  | `accept-lancedb-filter-policy-exception` evidence. On lancedb 0.37.1, the engine's literal builder mis-serialises exactly the two known value classes (apostrophe runs collapse `''` to one apostrophe; backslash-before-apostrophe emits `\'` undoubled) and no others. `translate_where` refuses both classes, accepts all 27 ordinary values with zero false refusals, and live-table equality filters match only their own row (28/28 pass). Supports ADR-058 (LanceDB filter policy exception) |
| 23       | [OCR routing gate calibration](./23-ocr-routing-gate-calibration-2026-09-07/)                    | 2026-09-07 | COMPLETE               | `improve-rag-input-quality-5` task 1.7. Calibration-set sweep of `OCR_FALLBACK_MIN_CONFIDENCE` × `OCR_FALLBACK_PAGE_FRACTION`: 40/48 cells zero-error; only degenerate `page_fraction=0.0` misroutes clean text (routes everything); scanned PDF carried at every grid point. Selected gate 0.5/0.5 (middle of admissible region; set cannot discriminate within it — stated as limitation) |
| 22       | [Raw-query Qwen3-Embedding-4B baseline](./22-raw-query-qwen4b-baseline-2026-09-07/)                | 2026-09-07 | COMPLETE (measurement)  | `improve-rag-input-quality-5` task 1.5. Stage 1 retrieval baseline, 10,024 FreshStack parents → 32,631 chunks in LanceDB, cloud embeddings, 223 queries, rerank off. Hybrid leads dense on every headline metric except R@1 (12.1% vs 12.9%); R@5 25.6% vs 22.7%; hybrid p95 1,984 ms. Not comparable with 9a (model/dims/store/granularity all differ). Index and corpus preserved outside git (`DATA_LOCATIONS.md`); `gate_noise_analysis.py` derived the task 1.6 noise tolerances |
| 24       | [OCR routing evaluation](./24-ocr-routing-eval-2026-09-08/)                                        | 2026-09-08 | PASS (run 2)            | `improve-rag-input-quality-5` task 5.1. Run 1 FAILED on the structure-marker gate — the original fixtures were 605-byte blank PDFs with nothing to OCR (commit `a7d7cd2`, honest negative). Task 1.1 repair replaced them with rasterised pages from a CC0 paper (Dashnow et al. 2014, attribution + sha256 in the fixtures manifest, commit `786055e`); gates unchanged. Run 2: all five gates PASS — 0 failures, 3/3 fast-path byte-identity, worker 156.9 s/page ≤ 180 (13% margin), routing 2.26 ms ≤ 50, headings recovered on both routed pages, ground-truth word recall 93.1%/94.3%. Full worker Markdown committed in `output/ablation.json` |
| 25       | [Token-aware chunking ablation](./25-token-chunking-ablation-2026-09-08/)                          | 2026-09-08 | PASS                    | `improve-rag-input-quality-5` task 5.2. Baseline splitter vs model-token Markdown chunker on the exp 22 corpus/queries. All four frozen gates PASS: R@5 0.2362 ≥ 0.231, identifier-heavy R@10 0.2775 ≥ 0.2583, embedded-token ratio 0.9749 ≤ 1.15, query p95 2,183 ms ≤ 2,850. Benefit is cost and input-contract correctness, not quality: largest chunk halved (2,087 → 1,120 payload tokens), 22,281 chunks vs 32,631. Promoted at task 5.5 — ADR-063 Accepted |
| 26       | [Query-instruction ablation](./26-query-instruction-ablation-2026-09-08/)                          | 2026-09-09 | FAIL (negative result)  | `improve-rag-input-quality-5` task 5.3. Raw vs instructed queries interleaved on the frozen exp 22 index (instruction excluded from index identity, task 4.8). Paired R@5 lift **−0.0246** against a frozen +0.0300; identifier-heavy R@10 0.2563 < 0.2583. Latency gate failed for BOTH arms on a slow provider day — the raw arm was the slower at p95 5,543 ms vs 3,543 ms — so it did not discriminate and carries no evidence against the instruction. Raw arm reproduced exp 22's R@5 to six decimals, pinning the delta on the instruction. `EMBEDDING__QUERY_INSTRUCTION` stays empty; text NOT retuned |
| 27       | [Combined candidate path](./27-combined-candidate-path-2026-09-09/)                                | 2026-09-09 | FAIL (negative result)  | `improve-rag-input-quality-5` task 5.4. 2×2 over chunking × query instruction; two cells loaded from committed checkpoints, two measured interleaved on the preserved exp 25 index. Combined R@5 0.2237 < 0.231 (FAIL); identifier-heavy R@10 0.2699 ≥ 0.2583 (PASS); p95 4,790 ms (FAIL, same provider drift — the raw chunking arm read 3,390 ms today vs exp 25's 2,183 ms yesterday). The failure is entirely the instruction: the chunker's own arm cleared the bar again at 0.2365. Interaction +0.0118 (sub-additive). Scope limit recorded: FreshStack has no PDFs, so OCR routing is not exercised |
| 28       | [PDF classification prevalence](./28-pdf-classification-prevalence-2026-09-09/)                    | 2026-09-09 | FAIL (negative result)  | `improve-rag-input-quality-5` task 5.5. Preregistered prevalence study (s-research template; deviations declared) over 79 de-duplicated real PDFs from the operator's Zotero library, 2,744 pages, zero parse failures, zero cost. Safety gate FAIL: one 991-page `mixed` document with 1,127 chars/page and only 10/991 pages flagged routed on the unconditional type rule — 29.3 wasted OCR hours, 1,375× slowdown on an 80-second corpus. Benefit gate FAIL: needs-OCR 2/79 = 2.5%, Wilson CI [0.7%, 8.8%] — unresolved at this n, NOT absent. Calibrated thresholds vindicated (caught doc_067, `text_based` at confidence 1.0 extracting zero characters); the unconditional `mixed` rule is the defect. `OCR_FALLBACK_ENABLED` stays `false`, thresholds stay at the 0.0 sentinels |
| 29       | [PDF routing repeat study](./29-pdf-routing-repeat-2026-09-13/)                                      | 2026-09-13 | PASS (recall gate not_evaluable by construction; dev_003 label amended) | `repeat-pdf-routing-study`. Preregistered repeat on an approved 17-doc collection (3 dev + 14 held-out, independent pypdf page labels, freeze commit before scoring). Candidate (post-`9bf4810`, 0.5/0.10) false-routes nothing held-out; the book stays fast-pathed while both pinned baseline arms still route it (30.5 h projected). Dev recall: Kerr routes. Sloman label corrected to needs_ocr=false post-review — it is a pdf-inspector extraction failure, not a scan (pypdf reads 2,944 chars/page); its dev route under the candidate was a false route on corrupted evidence, removed by the exp-30 reader chain |
| 30       | [Reader fallback chain](./30-reader-fallback-chain-2026-09-13/)                                       | 2026-09-13 | PASS (4/4 gates)                      | Validates the tiered fallback chain pdf-inspector → liteparse → pypdf against a script-local mirror of the historical pypdf-only guard (928f030) on 5 docs (2 IA GlyphLessFont scans, 1 WinAnsi, Kerr control, healthy control). liteparse tier rescues all 3 silent-empty docs; median-of-3 retries 0.05–0.20 s vs pypdf 0.31–11.5 s (~56× on the 136-page scan), omits textless pages (121/136), preserves every routing decision, never fires on healthy files. Second pdf-inspector failure mechanism recorded (IA GlyphLessFont). Evidence for ADR-066; shipped |
| 31       | [Reader rescue retrieval impact](./31-reader-rescue-retrieval-impact-2026-09-15/)                      | 2026-09-15 | PASS (decisive; negative for the candidate) | Three paired cells (pdf-inspector-only sanity, production LiteParse-first chain, pypdf-only guard mirror at 928f030) on 8 held-out natural silent-empty PDFs (6 IA GlyphLessFont, 2 ACL WinAnsi-no-/ToUnicode) + 11 healthy distractors, 24 poppler-labelled queries, frozen text-span matching, `documents` profile, local Ollama embeddings, OCR off, preflight 14/14. Both rescues retrieve gold evidence (Recall@10: B 0.583, C 0.792 vs A 0.000 by construction), but the chain is materially worse than the pypdf guard: paired hit@10 Δ −0.208, bootstrap 95% CI [−0.375, −0.083]. Four of five disagreements are parser-stage losses on the Type B ACL papers (LiteParse renders WinAnsi text at 0.18–0.39 8-gram overlap — rescued by character count, unreadable in content); one is a retrieval miss on fully recovered Type A evidence. Extraction speed still favours the chain ~7× (3.9 s vs 28.7 s median). Production unchanged; a fallback-order change needs a separate proposal |
| 32       | [Misnamed-extension content detection](./32-misnamed-extension-detection-2026-09-16/)                  | 2026-09-16 | PASS (5/5 gates)                      | Detection-only proof for `pin-magika-detection`/ADR-068 that the pinned Magika detector (`magika==1.0.3`, CLI transport) labels by content, not name. All 19 Experiment 31 corpus PDFs (8 silent-empty: 6 IA GlyphLessFont, 2 ACL WinAnsi; 11 healthy distractors) copied to lying `.py`/`.md`/`.txt` names still detect `document/pdf`; all 11 gate-corpus text/code files copied to `.pdf` keep their content-true labels (`code/python`, `document/markdown`, `document/text`). True-name baselines all-match (exit 0); swap sets exit 1 by design (shift reported). No fallback, timeout, or detector error; no ingest ran. Elapsed per set 0.24–0.86 s. Evidence regenerated 2026-09-16 with repo-relative paths (path-hygiene fix from PR review); verdict and gates unchanged |
| 33       | [OCR routing natural-positive study](./33-ocr-routing-natural-positive-2026-09-17/) | 2026-09-17 | FAIL (Stage A) | First measurement of OCR-routing recall on natural positives: 40 open-licence PDFs, 1,123 pages, operator-verified page labels (18 needs_ocr, 22 usable). Shipped gate catches 10/18 (recall 0.556, Wilson [0.337, 0.754]) and wrongly routes 1/22 (`mx08`, image_based classification). The `full-page-ocr-evidence` candidate (PR #95) catches 11/18 (0.611) for 49 extra OCR pages and no new false alarm. Three causes: pdf-inspector samples 8 pages (fixed by the candidate, `tl02`); its page test misses scanned pages carrying a header line or junk OCR text (`mx02`, `mx07`, `tl07`, `tl08`) and the ADR-066 rescue zeroes evidence after recovering junk (`rf06` 0.236, `rf07` 0.469 fast-path recall); and the gate never looks at equations (`bd04`). Operator review recorded a second construct — structure and understanding needs (equations, figures, columns, tables, non-Latin scripts) — kept as `understanding_label` |
