...
| 18       | [Ingestion lock-scope baseline and conditional A/B](./18-ingestion-lock-scope-ab-2026-08-19/)        | 2026-08-19 | ACTIVE              | Stage 3B measurement gate (design D12). Phase A: H1–H5 all PASS (bounded units, RSS ≤2× per 4× files, failure safety, swap, unchanged skip). Timing: single-stream lock wait ≈0 (sequential by construction); 2-stream contended with real Ollama embeddings is fully serialised — contender lock wait 95.6% of wall, speedup 1.002. Lock scope demonstrably blocks all concurrent embedding; Phase B A/B decides retain/reject of the minimal narrow-lock change |
