...
| 29       | [PDF routing repeat study](./29-pdf-routing-repeat-2026-09-13/)                                      | 2026-09-13 | PASS (disclosed vacuous recall gate) | `repeat-pdf-routing-study`. Preregistered repeat on an approved 17-doc collection (3 dev + 14 held-out, independent pypdf page labels, freeze commit before scoring). Candidate (post-`9bf4810`, 0.5/0.10) false-routes nothing; the book stays fast-pathed while both pinned baseline arms still route it (30.5 h projected). Dev recall: Kerr + Sloman both route. Held-out recall vacuous by construction — no unseen needs-OCR doc exists; disclosed. Enable-only arm exposes the sharp edge: Sloman missed at 0.0 sentinels. New finding: Sloman is a pdf-inspector extraction failure, not a scan (pypdf reads 2,944 chars/page) — follow-up flagged |
| 30       | [Reader fallback chain](./30-reader-fallback-chain-2026-09-13/)                                       | 2026-09-13 | PASS (4/4 gates)                      | Validates the tiered fallback chain pdf-inspector → liteparse → pypdf against the shipped pypdf-only guard on 5 docs (2 IA GlyphLessFont scans, 1 WinAnsi, Kerr control, healthy control). liteparse tier rescues all 3 silent-empty docs up to ~45× faster (0.36 s vs 16.1 s on the 136-page scan), omits textless pages (121/136), preserves every routing decision, never fires on healthy files. Second pdf-inspector failure mechanism recorded (IA GlyphLessFont). Evidence for ADR-066; implementation follows as an OpenSpec change |
