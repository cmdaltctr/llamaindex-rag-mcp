"""Experiment 26 evaluation runner: query-instruction ablation (task 5.3).
...
preserved Experiment 22 baseline index (read-only). Nothing is ingested
and no index is written: the query instruction deliberately stays out of
`build_index_identity` (task 4.8), so the same index serves both arms.
...
systematic first-call/second-call bias cannot land on one arm. Whether
the whole run shares ONE execution period is now recorded rather than
asserted: see the session state written next to the checkpoints.
...
Transient-provider handling follows Experiment 25: OpenRouter's free-tier
...
# Preserved Experiment 22 index (DATA_LOCATIONS.md). Read-only here.
STORE_URI = Path.home() / "Development/DATA/omrg/experiments/exp22-lancedb"
COLLECTION = "exp22"
TOP_K = 50

# Task 4.7 candidate, fixed verbatim before any measurement.
CANDIDATE_INSTRUCTION = (
    "Given a user query, retrieve passages that provide relevant and "
    "accurate evidence for answering the query."
)
CELLS = {"raw_none": "", "candidate_instruction": CANDIDATE_INSTRUCTION}
...
def resolve_checkpoint_base(limit: int | None = None, run_name: str | None = None) -> Path:
    ...
    the name must be a single safe path component. Without a destination
    the runner refuses: the legacy ``output/cells`` checkpoints are
    historical evidence, not a resume target.
    """
