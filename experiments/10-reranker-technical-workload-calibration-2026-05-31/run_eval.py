"""Run Experiment 10: Reranker technical-workload calibration.

Evaluates dense-only and hybrid retrieval under multiple reranker pool sizes
(max_fetch=50, 200, 500) plus rerank-off baselines on the FreshStack LangChain
corpus (reused from Experiment 9a).

Cell matrix: 2 modes × 4 rerank policies = 8 cells.
"""
...
def _load_ground_truth(path: Path) -> dict[str, Any]:
    """Load and validate ground truth from Experiment 9a."""
...
def _setup_environment(
    ...
    chroma_dir: Path,
    *,
    rerank_max_fetch: int | None = None,
    rerank_fetch_multiplier: int = 10,
) -> None:
    """Patch env vars and module globals for a specific cell configuration."""
    if mode == "dense-only":
    ...
    if rerank_max_fetch is not None:
    ...
    for mod_name in ("rag_mcp.config", "rag_mcp.retrieval", "rag_mcp.ingestion"):
        ...
        if rerank_max_fetch is not None:
            if hasattr(mod, "RERANK_MAX_FETCH"):
                mod.RERANK_MAX_FETCH = rerank_max_fetch
            if hasattr(mod, "RERANK_FETCH_MULTIPLIER"):
                mod.RERANK_FETCH_MULTIPLIER = rerank_fetch_multiplier
...
def _parent_id(result: dict[str, Any]) -> str:
    """Extract FreshStack parent ID from a retrieval result."""
...
def _rank_map(parent_ids: list[str]) -> dict[str, int]:
    """Build first-occurrence rank map from deduplicated parent IDs."""
    ranks: dict[str, int] = {}
...
def _alpha_ndcg(
    ...
    """Compute alpha-nDCG@k over answer nuggets."""
...
def _metrics_for_query(
    ...
    """Compute all quality metrics for a single query."""
    ranks = _rank_map(parent_ids)
    relevant = set(query.get("relevant_parent_ids") or [])
    nuggets = query.get("nuggets") or []

    # Coverage@20: fraction of nuggets with at least one relevant doc in top 20
    covered = 0
    ...
    *,
    ...
    for index, query in enumerate(queries, start=1):
        ...
        if index % 25 == 0 or index == len(queries):
            print(
                f"    {mode}/rerank={rerank}/pool={rerank_max_fetch}: "
...
def _result_payload(
    *,
    ...
    return {
        "experiment": "10-reranker-technical-workload-calibration-2026-05-31",
...
def _cell_key(mode: str, rerank: bool, rerank_max_fetch: int) -> str:
    """Unique key for a cell: mode + rerank + pool size."""
    if not rerank:
        return f"{mode}__rerank_off"
    return f"{mode}__rerank_pool_{rerank_max_fetch}"


def _load_checkpoint(path: Path) -> list[dict[str, Any]]:
    """Load previously completed cells from checkpoint."""
    if not path.exists():
        return []
    try:
        with path.open(encoding="utf-8") as f:
...
def _save_checkpoint(
    ...
    *,
    ...
    """Save checkpoint atomically after each cell."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run Experiment 10: reranker technical-workload calibration",
    )
    ...
    parser.add_argument("--rerank-cross", action="store_true",
                        help="Include rerank-off cells alongside rerank-on cells")
    parser.add_argument("--reranker-pools", nargs="+", type=int, default=[50, 200, 500],
                        help="RERANK_MAX_FETCH values to sweep (default: 50 200 500)")
    parser.add_argument("--rerank-fetch-multiplier", type=int, default=10,
                        help="RERANK_FETCH_MULTIPLIER (kept constant across cells)")
    parser.add_argument("--k-values", nargs="+", type=int, default=[5, 10, 20, 50])
    parser.add_argument("--warmup-queries", type=int, default=int(os.getenv("EXP10_WARMUP_QUERIES", "3")))
    parser.add_argument("--limit-queries", type=int, default=None)
    parser.add_argument("--resume", action="store_true",
                        help="Load checkpoint and skip completed cells")
    parser.add_argument("--no-resume", action="store_true",
                        help="Ignore any checkpoint even if it exists")
    ...
    reranker_pools = sorted(set(args.reranker_pools))
    rerank_fetch_multiplier = args.rerank_fetch_multiplier
    # When --rerank-cross is set, we include rerank=False cells as baselines
    ...
    if "rerank" not in sig.parameters:
    ...
    if args.resume and not args.no_resume:
        ...
        if cells:
            loaded = ", ".join(_cell_key(c["mode"], c["rerank"], c.get("rerank_max_fetch", 50)) for c in cells)
            print(f"Loaded checkpoint with completed cells: {loaded}", flush=True)
    completed = {
        _cell_key(c["mode"], c["rerank"], c.get("rerank_max_fetch", 50))
        for c in cells
    }
    ...
    for mode in modes:
        ...
        for rerank in rerank_settings:
            if not rerank:
                # Rerank-off cell: no pool sweep needed
                key = _cell_key(mode, rerank=False, rerank_max_fetch=0)
                if key in completed:
                    print(f"Skipping completed cell from checkpoint: {key}", flush=True)
                    continue
                print(f"Evaluating mode={mode}, rerank=off, top_k={top_k}", flush=True)
                ...
                # Rerank-on cells: sweep over pool sizes
                for pool_size in reranker_pools:
                    key = _cell_key(mode, rerank=True, rerank_max_fetch=pool_size)
                    if key in completed:
                        print(f"Skipping completed cell from checkpoint: {key}", flush=True)
                        continue
                    print(
                        f"Evaluating mode={mode}, rerank=on, pool={pool_size}, top_k={top_k}",
                        flush=True,
                    )
                    cells.append(_evaluate_cell(
                        mode=mode,
                        rerank=True,
                        rerank_max_fetch=pool_size,
                        rerank_fetch_multiplier=rerank_fetch_multiplier,
                        chroma_dir=chroma_dir,
                        queries=queries,
                        top_k=top_k,
    ...
    print(f"Total cells evaluated: {len(cells)}")
