...
The system SHALL cache recent query embeddings in process so that repeated
identical queries do not re-issue the embedding request to Ollama. The cache
SHALL be keyed by `(query, embedding_model_name)` and SHALL be bounded in
size. The cache SHALL apply uniformly across every retrieval branch in
`search()` — both the metadata-filtered direct-ChromaDB path and the
unfiltered path. An implementation that only catches one branch SHALL NOT
