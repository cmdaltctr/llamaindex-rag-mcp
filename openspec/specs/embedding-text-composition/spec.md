...
Define the contract governing exactly which text and which metadata keys reach
the embedding model, and how a change to that set invalidates previously
written vectors. The embedding text is an explicit, centrally-owned contract
rather than a by-product of which reader ran.
