...
Guard embedding vector norms at the ingest and query boundaries so that dense
ranking correctness rests on an enforced contract, not an undocumented
property of the embedding provider. Verified unit-norm behaviour (Ollama
2026-08-23 investigation; llama.cpp acceptance probe) is enforced by a
fail-closed guard at write time and a warn-once diagnostic at query time.
