...
Define the `omrg watch` command and file-system watcher behaviour for automatically ingesting, updating, and removing indexed document chunks as files change.
## Requirements
### Requirement: CLI subcommand `omrg watch`

The system SHALL provide a `omrg watch <path>` CLI subcommand that monitors a directory tree for supported document files and auto-ingests them into the vector-store index using the existing `ingest_path_async()` pipeline.

#### Scenario: Basic watch starts and waits
- **WHEN** the user runs `omrg watch /path/to/docs`
...
- **WHEN** the user runs `omrg watch /nonexistent/path`
...
- **WHEN** the user runs `omrg watch /path --debounce 5`
...
A watcher started by the login watcher installer SHALL invoke the existing `omrg watch` command and SHALL preserve the watch command's collection routing, debounce, recursive filtering, deletion handling, logging, and graceful shutdown semantics.
...
- **THEN** the launched process SHALL run `omrg watch` with `--collection research`
...
- **THEN** the launched process SHALL run `omrg watch` with `--debounce 5`
