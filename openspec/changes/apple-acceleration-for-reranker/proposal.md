...
The pluggable-reranker-backend change (PR #37) introduced a torch reranker backend behind an opt-in extra, opening the only untested route to Apple Silicon's GPU hardware: MPS (Metal Performance Shaders, the PyTorch GPU path). Experiment 16 (2026-08-03) already settled CoreML — it accelerated nothing because ONNX Runtime partitions the graph and most operations fall back to CPU. MPS has never been measured because PyTorch was not in the project at all. Now it is, behind the extra, and the question recurs every time someone asks why the reranker is CPU-only on a machine with a GPU.

This change runs Experiment 17 to close the question with numbers and records the verdict in ADR-039, so it stops recurring regardless of the outcome.
...
- Write `docs/adr/039-apple-acceleration-for-the-reranker.md` recording all three acceleration routes and their verdicts: CoreML closed by Exp 16, CPU as current default, MPS decided by Exp 17.
...
**Docs**
- New ADR-039 recording all three routes and their verdicts.
