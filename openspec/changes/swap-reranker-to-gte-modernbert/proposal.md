...
- Update `_select_onnx_variant()` to handle ModernBERT ONNX model paths (no ARM-quantised variant exists; use fp32 or dynamically quantised ONNX)
...
- **Dependencies**: No new runtime dependencies. `onnxruntime`, `transformers`, `huggingface-hub` remain. Model is ~300MB (vs ~23MB current) — first-use download time increases
...
- **Latency**: ~42ms per query (149M params, ModernBERT) vs ~90ms current (22.7M params, BERT) — paradoxically faster due to ModernBERT architecture optimisations, but larger model size means more memory
