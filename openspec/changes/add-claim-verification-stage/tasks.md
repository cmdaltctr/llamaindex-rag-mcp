...
- [ ] 1.1 Write `docs/adr/057-claim-verification-stage.md` recording the
...
- [ ] 1.2 Update the security review F4 status in
      `openspec/changes/add-grounded-answer-synthesis-3/security-review.md`
      to reference this ADR and the verification-stage change as the
      mitigation path.

## 2. Settings — add verification configuration

- [ ] 2.1 Add three fields to `core/answer/settings.py` (the pure-data
...
- [ ] 3.1 Add optional `answer.verify_claims`, `answer.verify_model`, and
...
- [ ] 3.2 Add env-var overrides (`ANSWER__VERIFY_CLAIMS`,
...
- [ ] 3.3 Document the profile override in `config/profiles/documents.yaml`
...
- [ ] 4.1 Create `core/answer/verify.py` with an async
...
- [ ] 4.2 Construct the judge prompt with evidence text wrapped in explicit
...
- [ ] 4.3 Add `compose.build_verify_llm(settings)` resolving through
...
- [ ] 4.4 Modify `core/answer/pipeline.py` to call the verification stage
...
- [ ] 4.5 Report `verification_ms` and `verification_calls` in diagnostics
      when diagnostics are enabled and verification ran, separate from
      `retrieval_ms` and `generation_ms`.
- [ ] 4.6 Confirm no file exceeds the 500-line ceiling; split cohesive
      helpers before editing the already-hot `pipeline.py` and `compose.py`.

## 5. Tests — unit and integration

- [ ] 5.1 Unit test: `verify_claims` with all-supported claims returns
      `verified=true` and keeps `status="ok"`.
- [ ] 5.2 Unit test: `verify_claims` with one unsupported claim returns
      `status="unverified_claims"`, lists the failing claim, and retains
      evidence and citations.
- [ ] 5.3 Unit test: verification disabled by default — no judge call is
      made, `status="ok"`, no `verified` field.
- [ ] 5.4 Unit test: verification provider unavailable (no API key) —
      `status="ok"` with `verification_skipped`, no judge call.
- [ ] 5.5 Unit test: judge call raises a network error — `status="ok"`
      with `verification_skipped`, evidence retained, pipeline does not
      raise.
- [ ] 5.6 Unit test: all verdicts unparseable — `status="ok"` with
      `verification_skipped` naming the unparseable rate.
- [ ] 5.7 Unit test: evidence text is wrapped in quote delimiters and the
      prompt labels it as untrusted data (prompt-injection resistance).
- [ ] 5.8 Unit test: profile override enables verification for a
      collection bound to a profile with `answer.verify_claims: true`.
- [ ] 5.9 Unit test: env var `ANSWER__VERIFY_CLAIMS=true` overrides a
      profile bundle that sets `answer.verify_claims: false`.
- [ ] 5.10 Integration test: the full answer pipeline with verification
...
- [ ] 5.11 Confirm `uv run pytest -m "not slow" --cov=rag_mcp` is green
       and coverage floors are held.

## 6. Documentation

- [ ] 6.1 Update `docs/guides/mcp-tools.md`: document the `verified` field,
...
- [ ] 6.2 Update `docs/guides/configuration.md`: document
...
- [ ] 6.3 Update `docs/guides/cli-reference.md`: document the `--verify`
       flag if exposed on the CLI `answer` command, or note that
       verification is settings-only.
- [ ] 6.4 Update `.env.example` with the three new settings (task 2.4
       covers the file edit; this task confirms the documentation is
       consistent).

## 7. OpenSpec validation

- [ ] 7.1 `openspec validate add-claim-verification-stage --strict` —
       passes.
- [ ] 7.2 `openspec validate --all --strict` — passes with no regressions
       on existing changes or specs.
- [ ] 7.3 `uv run lint-imports` — clean, no new ignore entries. If an
