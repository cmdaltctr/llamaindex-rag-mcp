...
- [x] 4.5 Commit the completed implementation with a Conventional Commit breaking marker, such as `feat!: remove rag-mcp compatibility alias`, and verify semantic-release will publish v3.0.0. (Committed as `4f02647` on `chore/remove-rag-mcp-residue`, forked from `v3` at `533b34a` with current version `2.2.0`; the `feat!:` marker maps to a major bump per `[tool.semantic_release]`, so the next release on the `v3` line publishes `3.0.0`.)
