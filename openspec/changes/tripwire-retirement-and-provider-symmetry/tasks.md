...
- [ ] 1.1 Rewrite the module docstring of `src/rag_mcp/config/legacy.py` to state
...
- [ ] 1.2 Replace the "LIFETIME: … removed in v3.0.0" comment block with a
...
- [ ] 1.3 Record in the docstring *why* flat entries are retained — pydantic
      cannot detect them (D1's measurement) — so the retention is not "cleaned
      up" by a future reader without re-deriving it.
- [ ] 1.4 Update CLAUDE.md gotcha #11 to reference the rule instead of restating
      the v3.0.0 trigger.
- [ ] 1.5 Append a dated forward-note (2026-08-07) to
...
- [ ] 2.1 Add `tests/test_docs_no_retired_env_vars.py`. Import
      `_RETIRED_ENV_VARS` from `rag_mcp.config.legacy` so the test cannot drift
      from the real mapping.
- [ ] 2.2 Scan operator-facing paths only: `.env.example`, `README.md`, and
...
- [ ] 2.3 Match assignment form `^\s*#?\s*NAME=` per line, where `NAME` is a key
...
- [ ] 2.4 On failure, report file, line number, the retired name, and its
...
- [ ] 2.5 Add a negative case asserting prose mentions do not trip the check
...
- [ ] 3.1 Fix `docs/guides/getting-started.md:54`: `RERANK_ENABLED=true` becomes
...
- [ ] 3.2 Confirm task 2 now passes with zero findings.
- [ ] 3.3 Update the 10 living `openspec/specs/*.md` files that name flat
...
- [ ] 3.4 Update `tests/TEST_README.md` where it names flat variables.

## 4. OpenRouter extraction backend

- [ ] 4.1 Create `src/rag_mcp/core/metadata/openrouter.py` and move
...
- [ ] 4.2 Update `core/metadata/registry.py:67` to register `openrouter` against
      the new module path.
- [ ] 4.3 Move the `_extract_openrouter_chat_async` entry in the PEP 562
      `_NAMES` map in `core/metadata/__init__.py` from `.extractor` to
      `.openrouter`.
- [ ] 4.4 Update patch targets in `tests/test_metadata_extractor.py` — the
...
- [ ] 4.5 Run the OpenRouter tests specifically and compare the passed count to
      ...
      deliberately broken.
- [ ] 4.6 Tasks 4.1-4.4 land in a single commit; the intermediate state has
      tests that pass without exercising the code.

## 5. OpenRouter LLM provider

- [ ] 5.1 Create `src/rag_mcp/core/providers/llm/openrouter.py` with a `build`
      ...
      literal.
- [ ] 5.2 Add the `register("openrouter", …)` line to
      `core/providers/llm/registry.py`.
- [ ] 5.3 Replace the inline `OpenAILike(...)` construction at
...
- [ ] 5.4 Delete `_CLOUD_BACKENDS = {"openrouter": "openrouter"}` from
      `extractor.py:78` together with its single caller at `extractor.py:57`.
- [ ] 5.5 Confirm the config surface is unchanged — `openrouter_llm_model`,
...
- [ ] 6.1 Add a test enumerating both registries and asserting `ollama`,
      `llamacpp`, and `openrouter` appear in each under the same name.
- [ ] 6.2 Assert no entry in the metadata extraction registry resolves to the
      dispatch module `core.metadata.extractor`.
- [ ] 6.3 Assert every sub-provider name accepted by config validation
