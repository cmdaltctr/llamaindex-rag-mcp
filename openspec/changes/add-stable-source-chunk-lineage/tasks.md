...
- [ ] 1.1 Add pure deterministic tests for `source_id`: same canonical path
...
- [ ] 1.2 Add pure deterministic tests for `chunk_id`: identical source
...
- [ ] 1.3 Add node-stamping tests for the shared `SOURCE` relationship,
...
- [ ] 1.4 Capture `MetadataMode.EMBED` and LLM-visible content before and after
  lineage stamping. Assert that identity/replacement metadata changes neither.
- [ ] 1.5 Add replacement regressions proving an identical forced re-ingest
...
- [ ] 1.6 Extend real ChromaDB and LanceDB contract fixtures to prove lineage
  persistence and source-scoped stale cleanup without backend-specific lineage
  code.
- [ ] 1.7 Add dense, BM25, hybrid-fusion, reranker-success, and reranker-failure
  tests proving stable lineage survives every result path while internal row
  IDs remain hidden from ordinary results.
- [ ] 1.8 Add listing, metadata-filter, deletion-preview, and path-deletion
  tests keyed by `source_id`, including equal-content sources at two paths.
- [ ] 1.9 Replace the pre-lineage auto-migration regression with a clean-boundary
...
- [ ] 2.1 Add `SOURCE_ID_KEY` and `CHUNK_ID_KEY` plus deterministic
  `build_source_id` and `build_chunk_id` helpers in
  `core/ingestion/source_state.py`. Retain `source_content_hash`; do not add a
  `document_hash` alias.
- [ ] 2.2 Keep source path canonicalisation consistent with the resolved
...
- [ ] 2.3 Refine the current stamping seam in `source_state.py` so stable
  ...
  hierarchy.
- [ ] 2.4 Derive `chunk_id` from `MetadataMode.NONE` text, stamp the exact
...
- [ ] 3.5 Detect rows for the same canonical `file_path` that lack or disagree
...
- [ ] 4.1 Update `core/ingestion/loader.py` so document listing groups by
  `source_id`, includes the human-readable path, and reports the chunk count.
- [ ] 4.2 Update deletion preview and `remove_document` in
...
- [ ] 5.3 Create an ADR after implementation confirms the formulas and
...
- [ ] 5.4 Run `uv run openspec validate add-stable-source-chunk-lineage --strict`
  and `uv run openspec validate --all --strict`.
- [ ] 5.5 Run the focused source-state, ingestion, replacement, deletion,
  retrieval, reranker, MCP, ChromaDB, LanceDB, import-contract, and file-size
  tests.
- [ ] 5.6 Run `uv sync`, Ruff check/format, `uv run lint-imports`, and
