...
When ingestion runs inside the MCP server's event loop, the loop SHALL remain able to service other tool calls. File reading, chunk splitting, ChromaDB writes, ChromaDB deletes, ChromaDB reads, and synchronous retrieval work SHALL NOT execute directly on the event-loop thread when they may block. A concurrent MCP `search` request issued while a long ingest is in flight SHALL receive a response without waiting for the ingest to complete.
