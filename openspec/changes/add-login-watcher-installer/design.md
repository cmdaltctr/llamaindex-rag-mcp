...
9. **A different watcher label on the same watch folder is refused with instructions and only removed on explicit consent (resolved during CodeRabbit review).**
   ...
   - Behaviour: existing-watcher detection reads each slug-matched candidate plist and compares the persisted watch path in `ProgramArguments[2]`, so unrelated folders sharing a name prefix (`docs` vs `docs-backup`) never match. A same-label re-run keeps plain overwrite semantics (confirm or `--force`). A different-label match refuses with the exact `launchctl bootout gui/<uid> <label>` and `rm <plist>` commands; an interactive confirmation or `--force` consents to automatic removal (best-effort bootout, then plist delete) before the new watcher is installed, so exactly one watcher remains. Covered by `TestDifferentLabelReplacement` and `TestFindExistingPlist`.
   - Alternative considered: auto-remove on `--force` without the refusal path. Rejected — the refusal message gives non-forcing users a manual route and makes the deletion explicit rather than a side effect.
