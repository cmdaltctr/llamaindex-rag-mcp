...
- [ ] 1.1 Confirm the registry names are still absent from `src/`:
...
- [ ] 1.2 Confirm the live registrations, which the rewrite in §3 must match:
...
- [ ] 1.3 Confirm the reranker default is still `False`:
      `grep -n "rerank_enabled" src/rag_mcp/core/retrieval/settings.py`
      → expect `rerank_enabled: LegacyBool = False` (~line 49).
- [ ] 1.4 Confirm `_dispatch_local_extraction` still dispatches via the registry
...
- [ ] 2.1 `docs/adr/026-provider-registry-and-openrouter.md` — append a
...
- [ ] 2.2 `docs/adr/027-local-cloud-provider-naming.md` — insert an inline
...
- [ ] 2.3 `docs/adr/027-local-cloud-provider-naming.md` — add a second inline
...
- [ ] 2.4 Verify no decision text changed: `git diff docs/adr/` MUST show
      additions only (no `-` lines other than trailing-whitespace normalisation).

## 3. Rewrite the providers guide registry section

- [ ] 3.1 `docs/guides/providers.md` — replace lines **110–126** (`## Registry
...
- [ ] 3.2 Rewrite the "Adding a new provider" steps. Correct procedure:
...
- [ ] 3.3 Confirm no dangling path remains:
      `grep -n "src/rag_mcp/config\.py" docs/guides/providers.md` → nothing.
- [ ] 3.4 Leave the "Shared classification knobs" blockquote (lines 103–108)
...
- [ ] 4.1 `docs/guides/getting-started.md:50` — replace "The reranker is enabled
...
- [ ] 4.2 `tests/TEST_README.md` — replace item 3 (lines **92–98**, "Module-level
...
- [ ] 4.3 `docs/guides/providers.md` — add the machine-readable provider block
...
- [ ] 5.1 Add `tests/test_docs_references.py`: scan `docs/guides/**/*.md` and
...
- [ ] 5.2 Extend `tests/test_registry_contract.py` with
...
- [ ] 5.3 Add a standing item to the change/ADR checklist: when a default value
      changes, grep `docs/guides/` for the old value. Record in design.md
      Decision 5 terms — a procedural partial, not automation.
- [ ] 5.4 Sanity-check that 5.1 and 5.2 actually fail on the rot: stash the §3–§4
      fixes, run both tests, confirm red; restore, confirm green. A prevention
      test never observed failing is not a prevention test.

## 6. Validate

- [ ] 6.1 `uv run openspec validate --all --strict`
- [ ] 6.2 `uv run pytest tests/test_docs_references.py tests/test_registry_contract.py -v`
- [ ] 6.3 `uv run pytest -m "not slow" --cov=rag_mcp` — confirm coverage floors
      hold. No `src/` change, so no coverage movement is expected; investigate
      any.
- [ ] 6.4 Re-run the §1 greps to confirm the docs now match the code.
- [ ] 6.5 Confirm the boundary held: `git diff` MUST NOT touch
