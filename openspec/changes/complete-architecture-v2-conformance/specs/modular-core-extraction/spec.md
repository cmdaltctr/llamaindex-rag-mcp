...
**Reason**: The compat shims were scheduled for removal in v2.0.0 and this
change ships v2.0.0. All fifteen shim modules (`server.py`, `cli.py`,
`watcher.py`, `azure_reader.py`, `ingestion.py`, `retrieval.py`,
`metadata_extractor.py`, `reranker.py`, `sparse_retriever.py`, and the six
files under `readers/`) have zero inbound imports from `src/`, carry no test
consumers, and require permanent exemptions in the coverage gate and the
file-size ceiling. Retaining them keeps a second live import path alongside
the agreed tree.

**Migration**: Update imports to the current paths —
...
`rag_mcp.ingestion` → `rag_mcp.core.ingestion`;
`rag_mcp.retrieval` → `rag_mcp.core.retrieval`;
`rag_mcp.metadata_extractor` → `rag_mcp.core.metadata`;
`rag_mcp.reranker` → `rag_mcp.core.retrieval.reranker`;
`rag_mcp.sparse_retriever` → `rag_mcp.core.retrieval.sparse`.
Archived `experiments/*/run_eval.py` scripts that import the old paths are
historical artefacts, are not executed in CI, and are deliberately left
unrepaired; each archived experiment directory carries a note recording the
v2.0.0 boundary.
