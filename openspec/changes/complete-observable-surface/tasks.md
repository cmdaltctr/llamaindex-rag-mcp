...
- [x] 1.1 Create `tests/test_openapi_conformance.py` with a helper that loads
...
- [x] 1.2 Add a helper deriving search parameters from
...
- [x] 1.3 Add a helper deriving listing keys by calling `list_documents` against a
...
- [x] 1.4 Add a helper deriving search-result keys twice against a stub store:
...
- [x] 1.5 Write the failing test: every derived search parameter appears in
      `SearchRequest.properties`. Confirm it fails naming `diagnostics`.
- [x] 1.6 Write the failing test: every derived listing key appears in
      `DocumentInfo.properties`. Confirm it fails naming `orphaned` and
      `source_id`.
- [x] 1.7 Write the failing test: every default search-result key appears in
...
- [x] 1.8 Write the failing test: every diagnostics-only result key is declared
      and NOT required.
- [x] 1.9 Write the default-comparison test implementing design D7's four
...
- [x] 1.10 Write the tests for rules 3a and 3b: the profile-resolved four
      ...
      "default 0.0".
- [x] 1.11 Add a test asserting the failure message names the missing field and
      its expected schema, so the check is actionable when it fires.

## 2. Thread A — close the gap

- [x] 2.1 Add `diagnostics` to `SearchRequest`: `type: boolean`,
      `default: false`, with a description stating it returns core-produced
      retrieval diagnostics.
- [x] 2.2 Add `source_id` to `DocumentInfo` as nullable string, described as the
      stable identity derived from the canonical source path.
- [x] 2.3 Add `orphaned` to `DocumentInfo` as nullable boolean, describing all
      three states and stating explicitly that it is machine-local.
- [x] 2.4 Add `metadata` (object) and the five lineage fields to `SearchResult`,
      marking every default-response field required.
- [x] 2.5 Add the diagnostic fields to `SearchResult` as optional: `id`,
...
- [x] 2.6 Rewrite the `similarity_threshold` description to state that the value
      ...
      `hybrid`.
- [x] 2.7 Run the Thread A tests and confirm all now pass.
- [x] 2.8 Confirm `transports/api/` still contains only `README.md` and
      `openapi.yaml` (the no-runtime-code scenario).
- [x] 2.9 Run the existing CI OpenAPI validation command locally to confirm the
      document is still a valid OpenAPI 3.1 spec after the edits.

## 3. Thread B — pin the timing contract (red first)

- [x] 3.1 Add tests to the retrieval diagnostics suite asserting a hybrid search
...
- [x] 3.2 Add a test asserting a dense-only search omits the sparse and fusion
      keys rather than reporting zero.
- [x] 3.3 Add a test asserting a non-reranked search omits the rerank key, and a
      reranked search includes it.
- [x] 3.4 Add a test asserting a search without diagnostics carries no `timings`
      field and matches the existing default result shape exactly.
- [x] 3.5 Add a test asserting identical result identities, order and scores with
      diagnostics on versus off, so timing is provably measurement-only.
- [x] 3.6 Add a test asserting a repeated identical query calls the embedding
...
- [x] 3.7 Add a test for the failed-rerank re-query: dense, sparse and fusion
...
- [x] 3.8 Add a test asserting a search returning zero results carries no
      timings, pinning the documented boundary rather than leaving it implied.

## 4. Thread B — implement stage timing

- [x] 4.1 Add a keyword-only `timing_report: dict | None = None` parameter to
...
- [x] 4.2 In `_dense_query_rows`, time the `_embed_query` call (wrapping the LRU
      lookup per design D4) and the `store.query_dense` call, writing each into
      `timing_report` when supplied.
- [x] 4.3 Add the same keyword-only parameter to the sparse runners in
      `core/retrieval/sparse_dispatch.py`, alongside the existing `report`
      argument, and record the sparse duration.
- [x] 4.4 In `_hybrid_query_rows`, accept and forward `timing_report` to both
      branches, and time the fusion call.
- [x] 4.5 In `search()`, create the timing dict, pass it to the dense-only and
      hybrid paths, and time the `reranker.rerank` call.
- [x] 4.6 Accumulate durations per stage rather than overwriting, so the
...
- [x] 4.7 Attach `timings` to each result row only when `include_diagnostics` is
      true, placing it alongside the existing per-row diagnostic fields so it
      survives fusion and reranking.
- [x] 4.8 Confirm no total is emitted, and that stage keys are omitted rather
      than zeroed when a stage did not run.
- [x] 4.9 Run the Thread B tests and confirm all now pass.

## 5. Verification

- [x] 5.1 Confirm `transports/mcp.py` and `transports/cli/search.py` are
      unmodified by this change (`git diff --stat` shows neither), proving the
      passthrough claim in the spec.
- [x] 5.2 Confirm `transports/mcp.py` is still under the 500-line ceiling by
      running `tests/test_file_size_ceiling.py`.
- [x] 5.3 Run `uv run pytest -m "not slow" --cov=rag_mcp` and confirm no
      regression against the coverage floors.
- [x] 5.4 Run the Tier 1 retrieval quality gate
...
- [x] 5.5 Run `uv run lint-imports` to confirm no new boundary violation.
- [x] 5.6 Run `openspec validate --all --strict`.
- [x] 5.7 Manually verify against a real collection:
...
- [x] 6.1 Update `docs/guides/mcp-tools.md` to document the `timings` field under
      the diagnostics section, including the concurrency caveat that per-stage
      durations do not sum to wall time.
- [x] 6.2 Update `docs/guides/cli-reference.md` for the same field under
      `--diagnostics`.
- [x] 6.3 Update `src/rag_mcp/transports/api/README.md` to state that the
      contract is conformance-checked against the implementation and to name the
      test that enforces it.
- [x] 6.4 Grep `docs/guides/` for any statement that the query path is
      uninstrumented and correct it (the documentation-drift check).
- [x] 6.5 Run `graphify update .` so the knowledge graph reflects the new
      modules and the changed retrieval call paths.
- [x] 6.6 Record a TDR only if an implementation surprise warrants one; this
