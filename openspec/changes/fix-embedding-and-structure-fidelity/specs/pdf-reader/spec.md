...
Each registered PDF reader SHALL declare the text format it emits as registry
metadata, alongside its existing import path and dependency probe. Downstream
consumers SHALL route on that declaration rather than inferring format from
...
- **WHEN** an operator inspects the configured reader
- **THEN** the system MUST be able to report whether page-level provenance is
