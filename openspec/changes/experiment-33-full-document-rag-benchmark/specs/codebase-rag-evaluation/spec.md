...
Experiment 33B SHALL evaluate the current OMRG `codebase` profile on a small frozen RepoProbe subset.
...
- **THEN** it SHALL NOT be included in the measured 33B baseline
...
- **AND** it SHALL NOT replace the 33B baseline
...
The first 33B baseline SHALL use only the approved small subset and SHALL run separately from 33A.

#### Scenario: Both profiles are scheduled concurrently on the Mac
- **WHEN** 33A is still running
- **THEN** 33B SHALL wait until 33A completes or stops