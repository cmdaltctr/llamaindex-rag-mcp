...
- **WHEN** `codebase_map.py` (now under `core/codebase/`) needs file-type
  detection
...
- **WHEN** `AGENTS.md` is read after the phase
- **THEN** every invariant and gotcha MUST reference the new module paths
  (e.g. `core/retrieval/reranker.py`, `compose.py`, `config.py` resolver)
- **AND** no invariant MUST describe the pre-refactor flat layout as current
