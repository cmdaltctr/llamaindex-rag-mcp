...
- [x] 1.1 Flip the three defaults in `src/omrg/config/__init__.py`:
  `ocr_fallback_enabled=True`,
  `ocr_fallback_min_confidence=0.5`,
  `ocr_fallback_page_fraction=0.10`.
- [x] 1.2 Mirror the same three defaults in
  `src/omrg/core/settings.py` (EffectiveSettings model) in the same
  commit.
- [x] 1.3 Update every test that asserts the old packaged defaults
...
- [x] 2.4 Check ADR-064 status line and add a superseded-by reference
  if the house style requires it (do not rewrite its body).

## 3. Verification

- [x] 3.1 `uv run pytest tests/test_settings_resolver.py tests/test_ocr_routing_gate.py tests/test_ocr_routing_seam.py tests/test_ocr_error_boundary.py tests/test_ocr_identity_reingestion.py -v` green.
- [x] 3.2 `uv run pytest -m "not slow" --cov=omrg` meets coverage
  floors; no test depends on a worker being present.
- [x] 3.3 `uv run openspec validate promote-ocr-fallback-defaults --strict` passes.
