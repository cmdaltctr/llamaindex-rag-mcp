...
regardless of whether reranking is active. The ÷30 threshold scaling factor
(ADR-021) SHALL be re-validated against the new default model
(`Alibaba-NLP/gte-reranker-modernbert-base`) before adoption. If the new
model's logit distribution differs significantly from
`cross-encoder/ms-marco-MiniLM-L-6-v2`, the scaling factor SHALL be
recalibrated via experiment.
...
#### Scenario: threshold scaling verified for new model
- **GIVEN** the default reranker model is `Alibaba-NLP/gte-reranker-modernbert-base`
- **WHEN** an A/B experiment compares threshold filtering behaviour between the old and new model
- **THEN** the experiment SHALL measure raw logit distributions for both models
- **AND** SHALL verify that the ÷30 scaling factor produces equivalent or better filtering behaviour
- **AND** SHALL recommend recalibration if logit distributions differ by more than 2× in standard deviation
