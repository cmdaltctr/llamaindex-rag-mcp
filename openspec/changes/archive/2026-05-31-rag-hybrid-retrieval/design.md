...
- **Risk: Experiment 9 silently evaluates dense-only code as hybrid if the implementation is incomplete.** → Mitigated by requiring `run_eval.py` to assert that `retrieval.search()` exposes the `hybrid` parameter and by removing any `TypeError` fallback that calls the pre-change dense-only signature.
- **Risk: a native sparse placeholder makes native hybrid silently behave like dense-only.** → Merge-blocking task 13.1 requires a real native sparse query, explicit BM25 fallback with WARNING, or a loud failure before evaluation. Returning an empty sparse side is not acceptable for a selected native backend.
- **Risk: hybrid diagnostics become an accidental public API.** → Merge-blocking task 13.3 requires deciding whether `dense_rank`, `sparse_rank`, `fused_rank`, `fused_score`, and `id` are public result fields or internal Experiment 9 diagnostics, then documenting or stripping them accordingly.
