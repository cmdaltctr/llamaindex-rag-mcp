...
- **Final verdict**: **BLOCKED** (first round, 2026-09-02) → **REMEDIATION COMPLETE — UNCONDITIONAL** after the second review round (same day; see the remediation record below). All ship gates are resolved.
- **Most important reason**: The new `metadata_filter` boundary reaches a string-built LanceDB SQL predicate. Project policy requires parameterised queries only and classifies any interpolated SQL as CRITICAL. **F1 RESOLVED 2026-09-02**: the user approved option (a) — a documented exception for the LanceDB filter adapter (ADR-058), with fail-closed verification as the standing control.
...
| F4 Referential-only `ok` | MEDIUM | **Mitigated — ADR-059 (claim-verification stage).** The citation guarantee is referential (ordinal → stored lineage), never semantic entailment. Experiment 20 (`experiments/20-citation-faithfulness-2026-09-02/`, PASS) measured an LLM judge (GLM-5.3) detecting unsupported claims with recall 1.0, false rejection 0.0, P95 3.33 s, and 100% cross-model agreement (DeepSeek V4 Flash 0731). The `add-claim-verification-stage` change implements the judge behind `ANSWER__VERIFY_CLAIMS=true` (opt-in cloud model, ADR-024 trade-off recorded in `docs/adr/059-claim-verification-stage.md`): all claims supported → `ok` + `verified=true`; any unsupported → `unverified_claims` with evidence retained; provider unavailable → `verification_skipped` and the referential-only `ok` remains the fallback. | `docs/adr/059-claim-verification-stage.md`, `docs/guides/mcp-tools.md` statuses, `experiments/20-citation-faithfulness-2026-09-02/results.md` |
...
**Follow-up (2026-09-02)**: Experiment 20 (`experiments/20-citation-faithfulness-2026-09-02/`, PASS) measured an LLM judge (GLM-5.3) detecting unsupported claims with recall 1.0, false rejection 0.0, P95 3.33 s, and 100% cross-model agreement (DeepSeek V4 Flash 0731 via OpenRouter). All pre-registered gates passed (H1: recall ≥ 0.80 and false rejection ≤ 0.10; H2: lexical gap ≥ 0.20; H3: P95 ≤ 5 s). A follow-up OpenSpec change will propose the claim-verification stage behind `ANSWER__VERIFY_CLAIMS=true`. The referential-only guarantee remains the fallback when verification is disabled. F4 status: **mitigated**.
...
**Verdict: REMEDIATION COMPLETE — UNCONDITIONAL** (second round, 2026-09-02)

All code-level findings from both review rounds are remediated and verified (see the remediation record; F7's pypdf upgrade landed 2026-09-02 by user direction, and the CLI-wide `--json` stream fix — `search`, `list`, `list-collections`, `delete`, `benchmark`, `profile`, and the shared Ollama helper — landed the same day, closing the residual noted in the second round). The F1 policy exception is resolved: option (a) approved 2026-09-02, ADR-058 accepted, Experiment 21 complete, and translator hardening landed. F4 (referential citations) is mitigated — experiment 20 (`experiments/20-citation-faithfulness-2026-09-02/`, PASS) confirmed an LLM judge detects unsupported claims with recall 1.0 and false rejection 0.0 across all adversarial classes, with 100% cross-model agreement. A follow-up OpenSpec change will propose the claim-verification stage behind `ANSWER__VERIFY_CLAIMS=true`.
