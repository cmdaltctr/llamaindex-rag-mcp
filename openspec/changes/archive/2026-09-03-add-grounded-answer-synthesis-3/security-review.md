...
| F4 Referential-only `ok` | MEDIUM | **Mitigated — experiment 20 PASS.** The citation guarantee is referential (ordinal → stored lineage), never semantic entailment. Experiment 20 (`experiments/20-citation-faithfulness-2026-09-02/`, PASS) measured an LLM judge (GLM-5.3) detecting unsupported claims with recall 1.0, false rejection 0.0, P95 3.33 s, and 100% cross-model agreement (DeepSeek V4 Flash 0731). All pre-registered gates passed. Decision: propose a claim-verification stage behind `ANSWER__VERIFY_CLAIMS=true` in a follow-up OpenSpec change. The referential-only guarantee remains the fallback when verification is disabled. | `docs/guides/mcp-tools.md` statuses, `experiments/20-citation-faithfulness-2026-09-02/results.md` |
