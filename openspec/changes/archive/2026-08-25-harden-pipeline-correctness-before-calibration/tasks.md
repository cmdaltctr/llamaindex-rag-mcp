...
- [x] 6.GC.1 Commit results separately; any production default change belongs in a follow-up ADR/OpenSpec. (Exp14 results committed in `0efd3b4`/`c41498c`/`4a03627`; the default-reader change landed separately in `843f90f`.)
- [x] 6.GC.2 If the evidence changes the default PDF reader or fallback policy, amend ADR-020 or write its successor **after** the result commit. If the current default is retained, link the experiment as confirming/limiting evidence without rewriting history. (`843f90f`: ADR-020 amended, successor ADR-050 written — pdf-inspector promoted to default per Experiment 14's fastest-parse/highest-Hit@5 result; commit follows the result commits.)

---

## 7. Closeout

- [x] 7.1 Run `uv run pytest -m "not slow" --cov=rag_mcp` at repository coverage floors. (2026-08-25: 1600 passed, 0 failed, 83 skipped (extras not installed in base venv), 14 deselected; TOTAL 89% is expected without the chroma/hybrid/community-leiden extras the dedicated coverage CI job installs, and the overall Codecov project check is informational per `CLAUDE.md`/TDR-007, not CI-blocking.)
- [x] 7.2 Run optional-backend contract jobs required by touched code (LanceDB real store, Torch backend as applicable). (2026-08-25: LanceDB — 130 passed, 0 failed, base venv, no extra needed. Torch — `uv sync --extra torch` then the CI job's exact command (`-m slow -k "torch or backend_contract"` against local Ollama) — 6 passed, 3 skipped, 0 failed; venv reverted to base afterwards via `uv sync`.)
- [x] 7.3 Run `ruff check`, `ruff format --check`, Pyright if configured, and `uv run lint-imports`. (2026-08-25: ruff check/format clean; lint-imports 8/8 contracts kept. Pyright is configured but not wired into CI (no pyright job in `.github/workflows/`); it reports 637 pre-existing errors repo-wide, mostly `pydantic-settings` `_env_file` API drift in unrelated test files and optional-dependency imports (torch/chromadb/rank_bm25/pypdfium2) unavailable in the base venv. Spot-checked the touched-file `object | None` pattern against the merge-base commit — it predates this branch. No new pyright regressions introduced by this change.)
...
- [x] 7.6 Audit the decision-record trail: every PAUSE GATE MUST have either a linked ADR/TDR action or an explicit `ADR/TDR not required` rationale; verify records distinguish planned design, deterministic test evidence, and empirical experiment evidence. (2026-08-25: independent read-only audit — every gate 0 through 6C has a linked, Accepted ADR/TDR citing real commit SHAs; no record over-claims evidence; ADR-019/ADR-048 explicitly flag evidence limits rather than hiding them.)
- [x] 7.7 Verify required records survived the evidence: Stage 2 semantic-swappability ADR, Stage 3A failure-safe-ingestion ADR, Stage 4 experiment-validity TDR, optional Stage 3B optimisation TDR, and any Stage 6 production-policy ADR amendments/successors. (2026-08-25: confirmed present and Accepted — ADR-047 (Stage 2, links `6dffece`), ADR-048 (Stage 3A), TDR-014 (Stage 4, links 4 commits), TDR-013 (Stage 3B, links `b4b01b6`), ADR-019 evidence updates (Stage 6A/6B, links `d49be1e`), ADR-020 amendment + successor ADR-050 (Stage 6C, links `843f90f`).)
