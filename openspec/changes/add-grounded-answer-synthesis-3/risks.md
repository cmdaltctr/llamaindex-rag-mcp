...
- **Remaining ship gates**: ~~(1) user approval of a documented policy exception for the LanceDB filter adapter~~ **resolved in principle 2026-09-02: the user chose acceptance (option a); the proposal `accept-lancedb-filter-policy-exception` is drafted, validates strict, and will be implemented and archived later.** ~~(2) ratification of the gitleaks false-positive triage~~ **resolved 2026-09-02: user accepted the 73 false positives; the deterministic floor no longer blocks.** The local Aikido re-scan is deferred by user decision (fix the MCP rules cache later; not a release gate).
