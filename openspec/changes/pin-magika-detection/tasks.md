...
- [ ] 2.6 Run focused new and affected regressions after implementation. Confirm fail-before and pass-after evidence. Base-install tests MUST fail when the package is absent; do not skip them. Simulate binary absence only in fallback tests.
- [ ] 2.7 Update dependency-floor tests only if the exact pin makes their measured rule require it. Do not add a speculative exemption; the exact pin has zero version gap.
