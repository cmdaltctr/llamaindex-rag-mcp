...
- **Per-provider timeout overrides for BOTH timeouts.** Add six optional fields on `MetadataSettings` — `{llamacpp,ollama,openrouter}_classify_timeout_override` and `{llamacpp,ollama,openrouter}_pipeline_timeout_override` — each `None` by default and falling back to the shared `classify_timeout` / `pipeline_timeout` respectively. Two resolvers map the active provider to its effective classify and pipeline timeout. Each is env-overridable per machine (e.g. `METADATA__LLAMACPP_PIPELINE_TIMEOUT_OVERRIDE`). The `_override` suffix avoids collision with the retired `METADATA__OLLAMA_CLASSIFY_TIMEOUT` tripwire. A new provider added later needs no timeout field to work — it inherits the shared default until someone tunes it.
- **Wire the resolvers into both consumption paths.** The direct-chat backends (`metadata/llamacpp.py`, `metadata/ollama.py`, `metadata/openrouter.py`) resolve their classify timeout per provider instead of reading the shared `classify_timeout` directly. The llamaindex pipeline (`metadata/llamaindex.py`) passes the per-provider-resolved pipeline timeout into `build()` instead of the shared `pipeline_timeout`.
