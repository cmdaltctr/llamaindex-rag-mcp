...
- [x] 4.2 Record whether real OCR measurement is authorised, including selected documents, timeout and runtime budget. If authorised, add and verify explicit execution tasks; otherwise label OCR cost as projected and recovery quality unmeasured. — NOT authorised; `results.md` labels all OCR figures as projections and recovery quality as unmeasured.
- [x] 4.3 Publish the routing comparison with missing-page evidence, hold-out results and limitations. Verify fixture-based time estimates are labelled, convenience selection is disclosed, and negative results are retained. — `results.md`: vacuous held-out recall gate disclosed, enable-only Sloman miss retained, selection disclosed.
