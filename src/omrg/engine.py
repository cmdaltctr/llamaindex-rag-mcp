...
class Engine:
    ...
    def __init__(
        ...
        *,
        ...
        answer_llm_factory: Any = None,
        verify_llm_factory: Any = None,
    ) -> None:
        ...
        Args:
            ...
            answer_llm_factory: Optional callable returning the answer LLM
                (resolved lazily on first ``answer()`` call).
            verify_llm_factory: Optional callable taking an answer block
                and returning the verify LLM.
        """
    ...
    async def answer(
        ...
        *,
        ...
        # Build the verify LLM if claim verification is enabled.
    ...
    def close(self) -> None:
        """Release this engine's resources.

        Closes the owned store, drops the query-embedding cache, and
        evicts only the BM25 cache entries in this store's identity
        namespace. Never touches process-wide ingestion coordination
        state. Other engines remain functional.
        """
        if self._closed:
            return
        self._closed = True
    ...
    def _resolve_settings(self, collection_name: str) -> Any:
        ...
        (matching the pre-engine behaviour where transports built it
        on demand).
        """
        ...
        if self._profile_resolver is not None:
            ...
            except Exception:  # noqa: S110 - profile resolution is best-effort
                pass
