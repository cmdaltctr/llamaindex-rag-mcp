...
The reranker is a plain class constructed by ``omrg.compose`` with
...
# consumer was the ``omrg.reranker`` v1 shim, itself deleted in the same
...
class CrossEncoderReranker:
    ...
    Constructed by the composition root (``omrg.compose``) with an
