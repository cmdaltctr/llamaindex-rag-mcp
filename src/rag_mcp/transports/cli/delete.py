...
def _delete_by_metadata(metadata, coll_name, dry_run, preview_delete, remove_by_metadata) -> dict:
    ...
    if dry_run:
        result = preview_delete(
            metadata_filter=metadata_filter, collection_name=coll_name,
        )
...
def _print_delete_result(result, coll_name, json_output, dry_run) -> None:
    ...
    if result.get("mode") == "collection":
        console.print(
            f"[green]✓[/green] Collection '[bold]{coll_name}[/bold]' deleted."
        )
    else:
...
def delete(
    path: Optional[str] = typer.Option(
        None, "--path", "-p", help="Source file path to delete chunks for.",
    ),
    ...
    if flags_provided == 0:
        console.print(
            "[red]Error:[/red] Provide one of: --path, --metadata, --collection."
        )
        raise typer.Exit(code=1)
    if flags_provided > 1:
        console.print(
