...
<!-- integration-inventory:start count=20 -->
...
| `omrg.integrations.ocr_worker.protocol` | Native                                                       | None                                                                | Versioned JSON Lines envelopes                 | None                                                                              | Wire-protocol twin; tests keep its independent copy compatible      |
...
| `omrg.integrations.pdf.page_routing` | Base dependency (`pdf-inspector`) | `OCR_ROUTING_UNIT=page` (opt-in; `document` is the default) | `page_evidence`, `merge_pages` | Full page scan raises — the page unit has no sampled evidence to fall back to | Page-unit OCR routing: per-page evidence and the merge back to one document |
...
| `omrg.integrations.tokenizer`         | Base `tokenizers` and `huggingface-hub` dependencies           | `EMBEDDING__TOKENIZER_MODEL` and `EMBEDDING__TOKENIZER_REVISION`    | `load_tokenizer(model, revision)`             | `core/chunking/model_token.py` legacy splitter                                    | Offline cache adapter; never downloads                              |
